...
def signup():
    ...
    if form.validate_on_submit():
        ...
        send_email(
            user.email,
            'Confirm Your Account on PulsePod',
            'auth/email/confirm',
            user=user,
            token=token
        )
...
def confirm(token):
    ...
    if current_user.confirm(token):
        ...
        flash('The confirmation link is invalid or has experied.', 'warning')
    return redirect(url_for('main.index'))
...
def resend_confirmation():
    ...
    flash('A new confirmation message has been sent to you by email',
          'warning')
    return redirect(url_for('main.index'))
